...
import { I18nProvider, TranslateText, Locale, useTranslate } from "./i18n";
import "./i18n/setup";
...
const ExampleInner = () => {
  ...
  const { changeLocale, i18n } = useTranslate();
        ...
        <div>
          ...
          <p>
            <TranslateText
              ...
              components={{ b: <button onClick={handleCreateTask}></button> }}
            />
          </p>
        </div>
      );
  ...
  return (
    <div style={{ margin: 12, maxWidth: 500 }}>
      <select value={i18n.locale} onChange={updateLang} style={{ marginBottom: 8 }}>
  ...
  return (
    <form onSubmit={handleSubmit}>
      <input
        ...
        type="text"
        placeholder={i18n.t({ id: "placeholder" })}
      />
      <button>
...
const TodoList = ({ items, onToggleTodoDone, onDeleteTodo }: TodoListProps) => {
  ...
  return (
    <div>
      <div>{i18n.t({ id: "items_info", values: { count: items.length } })}</div>
      {items.map((item) => (
        <TodoItem
          key={item.id}
          item={item}
...
const TodoItem = ({ item, onDelete, onToggleDone }: TodoItemProps) => {
  ...
  return (
    ...
    >
      ...
      <button onClick={() => onDelete(item.id)}>{i18n.t({ id: "delete_button_text" })}</button>
    </div>
  );
};
